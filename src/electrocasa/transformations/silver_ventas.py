...
@dp.table(
    ...
    comment="Ventas limpias y estandarizadas"
)
def silver_ventas():
    ...
    return (
        ...
        # Unificar los dos formatos de fecha
        .withColumn(
            "fecha_venta",
            coalesce(to_date(col("fecha_venta"), "yyyy-MM-dd"),to_date(col("fecha_venta"), "dd/MM/yyyy"))
        )
        ...
        .withColumn(
            ...
            .when(lower(trim(col("metodo_pago"))).isin("tarjeta de credito","tarjeta_credito","tarjeta","tc"),"tarjeta_credito")
            .when(lower(trim(col("metodo_pago"))).isin("transferencia","transferencia bancaria"),"transferencia")
